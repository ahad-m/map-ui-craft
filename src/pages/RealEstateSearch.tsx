...
import { Search, MapPin, MessageCircle, SlidersHorizontal, X, Sparkles, Languages, ArrowLeft, Bed, Bath, Maximize, School, GraduationCap, Check, ChevronsUpDown, Heart, Bot, Send, Loader2, LogOut, RotateCw } from 'lucide-react';
...
const RealEstateSearch = () => {
  ...
  useEffect(() => {
    ...
    if (chatSearchResults.length > 0) {
      ...
      setChatbotProperties(chatSearchResults); // (لا يزال مفيداً للـ baseProperties)
  ...
  useEffect(() => {
    if (!mapRef.current) return;
    console.log('🗺️ Map useEffect triggered:', { showChatbotResults, chatbotPropertiesLength: chatSearchResults.length }); // <-- تعديل
    if (showChatbotResults && chatSearchResults.length > 0) { // <-- تعديل
      ...
      <div className="relative h-screen w-full overflow-hidden">
        <div className="absolute inset-0">
          ...
          >
            ...
            {hasSearched && (filters.selectedSchool || (chatSearchResults.length > 0 && messages.some(m => m.criteria?.school_requirements?.required))) && nearbySchools.map((school) => (
              <AdvancedMarker
            ...
            {hasSearched && (filters.selectedUniversity || (chatSearchResults.length > 0 && messages.some(m => m.criteria?.university_requirements?.required))) && nearbyUniversities.map((university) => (
              <AdvancedMarker
